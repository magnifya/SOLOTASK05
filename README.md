...
- **失败回滚**：激活任一环节失败，回滚份额文件、公钥与轮换状态并
  清理备份；服务启动时先把崩溃残留的未完成激活回滚为 `prepared`，
  轮换状态跨重启持久。
- **提交点即激活事件**：份额轮换激活是一个可恢复事务，
  `share_rotation_activated` 事件是否落盘是唯一提交判据（不依赖
  轮换状态文件）。若进程在元数据/份额已写入、**事件尚未落盘**时退出
  （此时记录可能已停在 `activating` 甚至 `active`），重启恢复旧公钥与
  旧份额、把轮换置回 `prepared`，并保留经校验有效的暂存份额以便重试；
  若**激活事件已经落盘**，即使暂存或备份尚未清理，也必须保持 `active`，
  按事件把钱包公钥/在用份额确定性前滚补齐并清理全部残留，且绝不重复
  记事件。无法对账为一致状态（如已切换却既无激活事件又无旧份额备份）
  时恢复失败，服务拒绝就绪，绝不静默对外暴露半完成状态。
...
- 启动恢复在每个钱包的事务锁内扫描轮换残留：仅当轮换记录为
  ...
  用钱包，不留私钥副本。
- 崩溃时停留在 `activating`、或记录已写成 `active` 但**激活事件尚未
  落盘**的现场，统一回滚为 `prepared`（恢复原钱包元数据与旧份额、删除
  换入的新份额、清理备份、保留有效暂存）；**激活事件已落盘**的现场
  确定性前滚为 `active`（补齐在用新份额与新公钥）并清理暂存与备份，
  不重复记事件。恢复与孤儿清理不产生审计事件，审计 seq 跨重启接续、
  连续不重号。
- 轮换与资产两类恢复都在**每钱包事务锁内**完成：除启动扫描外，常驻
  进程在持锁的钱包/轮换查询、激活、签名、资产查询/创建/提交前先自愈
  他进程崩溃遗留的现场。因此激活与签名、资产提交、审批操作跨进程
  交错时按钱包事务锁串行，任何 HTTP/CLI 访问都读不到半完成状态；
  已首签请求重放仍 `200`，未首签请求激活后只接受当前份额。
- 恢复失败（现场无法对账为一致状态）**阻止服务就绪**：`WalletService`
  构造期恢复抛错则服务不启动，`serve` 以非零码 + 单行 JSON 失败退出；
  常驻进程锁内自愈遇到无法调和的现场时该请求返回 `500`，绝不静默跳过
  或返回半切换的公钥/份额。
