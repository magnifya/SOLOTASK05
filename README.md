...
登记期间残留的半截 `.<id>.json.tmp` 由该钱包锁域收敛——正式记录已在时，
下一次持锁访问在闭集校验（确定性命名、普通文件、非链接）后清理该残留，
正式记录逐字节不动；无正式记录时由下一次登记原子续作（先解链再
O_EXCL），中断可重试且不改业务，缺记录只登记一次。符号链接、子目录、
激活备份（`*.bak.json`）、随机临时名（`.tmp-*`、裸 `*.tmp`）或任何非法
命名一律不可对账：常驻请求 `503`、`serve` 拒绝就绪，**保留现场**、不猜写、
不删除。记录文件恰含 `wallet_id`、`snapshots` 两键，`snapshots` 为
