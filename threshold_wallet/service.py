...
class WalletService:
    ...
    def _reconcile_chain_state_locked(self, wallet_id: str) -> dict:
        ...
        重放跨链确认与多源仲裁的策略、票、操作及相邻提交事件，以及跨链
        派发事件：畸形票、未知操作票、重复来源、错误 state、孤 adopted
        票、畸形/重复/矛盾派发或提交关系矛盾都抛 RecoveryError
        （fail-closed，保留现场），审计 JSON 损坏抛
    ...
    def _recover_wallet(self, wallet_id: str) -> None:
        ...
        try:
            ...
            # 对账：逐事件重放，矛盾/损坏 fail-closed；纯只读。
            self._reconcile_chain_dispatch_events(wallet_id)
    ...
    def _dispatch_approval_message(
        ...
        """dispatch 审批单 message 必须逐字一致的紧凑 JSON（无空格、键序
    ...
    def _chain_dispatch_events_strict(self, wallet_id: str) -> list[dict]:
        ...
        固定、各值合法、state 恒为 requested）。

        这里只做与现场无关的形状校验；与账本/策略/审批单的语义复核在
        :meth:`_reconcile_chain_dispatch_events` 按事件 seq 完成。同一
        dispatch_id 或同一 operation_id 的重复提交点、任何形状畸形都是
        不可对账现场（RecoveryError）。纯只读，不分配 seq。"""
        # 用 events_by_type 而非按 request_id 分组：后者会丢掉 request_id
        # 为 null/非字符串的畸形事件，必须让它们也进入严格校验而非被静默
        # 忽略。
        events = self._audit.events_by_type(
        ...
        for event in events:
            ...
            if details["dispatch_id"] != dispatch_id:
                raise RecoveryError(
                    f"wallet {wallet_id!r} chain_dispatch_requested "
                    f"{dispatch_id!r} details do not match its request_id"
                )
            for key in ("operation_id", "adapter_id", "chain_id"):
                if not isinstance(details[key], str) or not ROTATION_ID_RE.match(
                    details[key]
                ):
                    raise RecoveryError(
                        f"wallet {wallet_id!r} chain_dispatch_requested "
                        f"{dispatch_id!r} has a malformed {key}"
                    )
            if details["state"] != "requested":
                raise RecoveryError(
                    f"wallet {wallet_id!r} chain_dispatch_requested "
                    f"{dispatch_id!r} has a non-requested state"
                )
    ...
    def _reconcile_chain_dispatch_events(self, wallet_id: str) -> None:
        """chain_dispatch_requested 事件与账本/策略/审批单的严格对账
        （调用方须持钱包事务锁）。

        每条事件都以其**提交之前**的现场复核在线首提的全部前置：

        - details.operation_id 指向账本中存在的操作，且事件提交时该操作
          尚未 committed（仍为 pending）；
        - 事件提交时该资产的链策略已配置且 enabled，details.chain_id 与
          当时策略链一致；
        - 同钱包审批单 actor_id 存在，message 逐字为按
          operation_id,dispatch_id,adapter_id,chain_id 序的紧凑 JSON，
          且状态为 approved（其后推进为 signed 亦认可）。

        任一不满足都是不可对账现场（RecoveryError，fail-closed，保留
        现场）。纯只读，不记事件、不改 seq、不写状态。"""
        events = self._chain_dispatch_events_strict(wallet_id)
        if not events:
            return
        ledger = self._store.check_asset_ledger_semantics(wallet_id)
        operations = ledger["operations"]
        dispatch_by_seq = {event["seq"]: event for event in events}
        # 按 seq 重放全部事件，取各派发事件提交之前的策略与操作终态现场。
        policies: dict[str, dict] = {}
        committed: set[str] = set()
        for event in self._audit.all_events(wallet_id):
            ...
            if event_type == audit.TYPE_CHAIN_POLICY:
                ...
                if isinstance(op_id, str):
                    committed.add(op_id)
            dispatch = dispatch_by_seq.get(event["seq"])
            if dispatch is None:
                continue
            ...
            if record is None:
                raise RecoveryError(
                    f"wallet {wallet_id!r} chain_dispatch_requested "
                    f"{dispatch['request_id']!r} refers to unknown asset "
                    f"operation {operation_id!r}"
                )
            if operation_id in committed:
                raise RecoveryError(
                    f"wallet {wallet_id!r} chain_dispatch_requested "
                    f"{dispatch['request_id']!r} was committed after the "
                    f"asset operation {operation_id!r} reached its final "
                    "state"
                )
            policy = policies.get(record["asset_id"])
            if policy is None or not policy["enabled"]:
                raise RecoveryError(
                    f"wallet {wallet_id!r} chain_dispatch_requested "
                    f"{dispatch['request_id']!r} has no enabled chain "
                    "policy before it"
                )
            if details["chain_id"] != policy["chain_id"]:
                raise RecoveryError(
                    f"wallet {wallet_id!r} chain_dispatch_requested "
                    f"{dispatch['request_id']!r} is on a different chain "
                    "than the policy in effect"
                )
            # --- 审批单复核（同钱包、approved、message 逐字一致）---
            try:
                approval = self._store.get_request(
                    wallet_id, dispatch["actor_id"]
                )
            except CorruptDataError:
                raise
            except ValueError as exc:
                raise RecoveryError(
                    f"wallet {wallet_id!r} chain_dispatch_requested "
                    f"{dispatch['request_id']!r} approval record is "
                    "unreadable"
                ) from exc
            if not isinstance(approval, dict):
                raise RecoveryError(
                    f"wallet {wallet_id!r} chain_dispatch_requested "
                    f"{dispatch['request_id']!r} refers to an unknown "
                    "approval request"
                )
            ...
            if approval.get("message") != expected_message:
                raise RecoveryError(
                    f"wallet {wallet_id!r} chain_dispatch_requested "
                    f"{dispatch['request_id']!r} approval message does "
                    "not match"
                )
            # 提交时必为 approved；其后该审批单只能停留 approved 或经
            # /sign 推进为 signed（pending 才可能转 rejected/expired），
            ...
            if approval.get("state") not in ("approved", "signed"):
                raise RecoveryError(
                    f"wallet {wallet_id!r} chain_dispatch_requested "
                    f"{dispatch['request_id']!r} approval request is not "
                    "approved"
                )
    ...
    def post_chain_dispatch(
        ...
        """提交某资产操作的跨链派发请求。返回 (HTTP 状态码, 视图 V)。

        请求体恰含 dispatch_id,adapter_id,approval_request_id 三键
        （HTTP 边界拦键集）：三者与路径 operation_id 均为安全标识，非
        安全 ID 一律 400；钱包/操作/链策略/同钱包审批单任一未知 404。

        首提前置：操作仍为 pending、该资产链策略已启用、审批单经锁内
        懒过期后为 approved 且 message 逐字为按
        operation_id,dispatch_id,adapter_id,chain_id 序的紧凑 JSON；
        任一不满足 409 且现场不变。chain_id 取策略链。成功 201 返回
        V={dispatch_id,operation_id,adapter_id,chain_id,state=requested}
        （键序如列）。同 dispatch_id 同参重放 200 同 V（优先于状态与
        审批复查），异参或该操作已有派发 409。

        chain_dispatch_requested 是唯一提交点（request_id=dispatch_id、
        actor_id=approval_request_id、reason=null、details=V）：在每
        钱包跨进程事务锁内追加，事件之外不写任何状态文件；跨进程并发
        只有一个 201，失败/重放不记该事件。恢复检查、校验、状态判定与
        事件追加全部在锁内完成。
        """
        try:
            with self._wallet_lock(wallet_id):
                ...
                committed = self._chain_dispatch_events_strict(wallet_id)
                # 幂等优先于 404/状态判定：已提交的同 dispatch_id 重放。
                existing = next(
                    (
                        event
                        for event in committed
                        if event["request_id"] == dispatch_id
                    ),
                    None,
                )
                if existing is not None:
                    saved = existing["details"]
                    if (
                        saved["operation_id"] == operation_id
                        and saved["adapter_id"] == adapter_id
                        and existing["actor_id"] == approval_request_id
                    ):
                        # 同参（含审批单标识）重放：200 同 V，不复查现状
                        return 200, dict(saved)
                    raise ServiceError(
                ...
                # 审批门控：同钱包既有审批单，经锁内懒过期（可能原子记一次
                # request_expired）后须为 approved，message 逐字一致。
                approval = self._expire_if_needed(wallet_id, approval)
                if approval["state"] != "approved":
                    raise ServiceError(
                        409,
